...
function workerJob(value: unknown): WorkerJobResponse {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new ApiError(502, "Worker returned an invalid job response.");
  ...
  if (
    ...
    throw new ApiError(502, "Worker returned an invalid job response.");
  if (data.output_path !== null && typeof data.output_path !== "string")
    throw new ApiError(502, "Worker returned an invalid output path.");
  if (
    data.audio_duration !== null &&
    (typeof data.audio_duration !== "number" || data.audio_duration <= 0)
  )
...
export async function workerReady(): Promise<boolean> {
  ...
  return Boolean(
    result && typeof result === "object" && "ready" in result && result.ready,
  );
}
...
export async function cancelWorkerJob(id: string): Promise<WorkerJobResponse> {
  return workerJob(
    await callWorker(`/v1/jobs/${encodeURIComponent(id)}/cancel`, {
      method: "POST",
      body: "{}",
    }),
  );
...
export async function uploadWorkerReference(
  id: string,
  sourcePath: string,
): Promise<string> {
  const audio = await readFile(sourcePath);
  if (audio.byteLength < 1 || audio.byteLength > 20 * 1024 * 1024)
    throw new ApiError(
      422,
      "Reference audio must be between 1 byte and 20 MB.",
    );
  const extension = extname(sourcePath).toLowerCase();
  if (!/^\.[a-z0-9]{2,5}$/.test(extension))
    throw new ApiError(422, "Reference audio extension is invalid.");
  const response = await workerFetch(
    `/v1/references/${encodeURIComponent(id)}`,
    {
      ...
      headers: {
        ...
        "x-reference-extension": extension,
      },
  ...
  if (!response.ok) {
    const detail =
      payload && typeof payload === "object" && "detail" in payload
        ? String(payload.detail)
        : "Worker reference upload failed.";
    throw new ApiError(response.status, detail);
  }
  if (
    !payload ||
    ...
    throw new ApiError(502, "Worker returned an invalid reference path.");
  return payload.path;
}
...
export async function downloadWorkerAudio(id: string): Promise<Uint8Array> {
  ...
  if (!response.ok) {
    ...
    const detail =
      payload && typeof payload === "object" && "detail" in payload
        ? String(payload.detail)
        : "Worker audio download failed.";
    throw new ApiError(response.status, detail);
  }
  const contentType = response.headers.get("content-type") || "";
  if (!contentType.toLowerCase().startsWith("audio/"))
    throw new ApiError(502, "Worker returned a non-audio result.");
  const declaredLength = Number(response.headers.get("content-length") || "0");
  if (declaredLength > MAX_WORKER_AUDIO_BYTES)
    throw new ApiError(502, "Worker audio exceeded the 100 MB limit.");
  const audio = new Uint8Array(await response.arrayBuffer());
  if (audio.byteLength < 1 || audio.byteLength > MAX_WORKER_AUDIO_BYTES)
    throw new ApiError(
      502,
      "Worker returned an empty or oversized audio result.",
    );
  return audio;
}
