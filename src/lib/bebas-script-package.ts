...
export function parseBebasScriptPackage(
  ...
  if (
    data.expressionDialect !== LEGACY_EXPRESSION_DIALECT &&
    data.expressionDialect !== EXPRESSION_DIALECT
  )
    packageError(
      `expressionDialect harus \"${LEGACY_EXPRESSION_DIALECT}\" atau \"${EXPRESSION_DIALECT}\".`,
    );
  ...
  const expressionDialect = data.expressionDialect as ExpressionDialect;
  const compiled = compileExpressionScript(script, expressionDialect);
