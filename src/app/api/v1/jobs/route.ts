...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!validId(idempotencyKey)) throw new ApiError(422, "Idempotency-Key must contain 8–100 letters, numbers, underscores, or hyphens.");
    ...
    if (!["normal", "failure", "slow"].includes(scenario)) throw new ApiError(422, "x-demo-scenario is invalid.");
    const selection = await appStore.mutate((state) => {
      ...
      const voice = synthesis.mode === "clone" || synthesis.mode === "hifi" ? state.voices.find((item) => item.id === synthesis.voiceId) : undefined;
      if ((synthesis.mode === "clone" || synthesis.mode === "hifi") && !voice) throw new ApiError(422, "The selected reference voice does not exist on the application server.");
      ...
      const job: ServerJob = {
        ...
        requestHash: digest,
        request: synthesis,
        voiceName: voice?.name ?? (synthesis.mode === "design" ? "Voice Design" : "Built-in voice contract"),
        status: "queued",
        progress: 0,
        createdAt: now,
        updatedAt: now,
    ...
    try {
      ...
      return NextResponse.json({ job: publicJob(updated!), created: true }, { status: 202 });
    } catch (error) {
      await appStore.updateJob(selection.job.id, { status: "failed", message: error instanceof Error ? error.message : "Worker submission failed.", updatedAt: new Date().toISOString() });
      throw error;
    }
  } catch (error) { return errorResponse(error); }
}
