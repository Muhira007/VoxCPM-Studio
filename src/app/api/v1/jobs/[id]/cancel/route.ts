...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (local.status !== "queued" && local.status !== "running") return NextResponse.json({ job: publicJob(local) });
    const worker = await cancelWorkerJob(id);
    const updated = await appStore.updateJob(id, { status: worker.status, progress: worker.progress, message: worker.message || "Job cancelled.", updatedAt: new Date().toISOString() });
