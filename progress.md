...
- Referensi saat publikasi awal (arsip): `ghcr.io/muhira007/voxcpm-studio-worker@sha256:90ba964343f769a428259a59ac0acd8523de82c02f4ffddd82e2e8d78d715fbd`.
...
**Pembaruan image ekspresif, 23 September 2026:** implementasi Fase 6B diterbitkan melalui [GPU worker image run 35856643949](https://github.com/Muhira007/VoxCPM-Studio/actions/runs/35856643949) dengan tag `sha-215d65a8c8689c8ab2d969cc24c1704aa8e8a5b9`. Referensi deployment aktif adalah `ghcr.io/muhira007/voxcpm-studio-worker@sha256:763938c78e0d1be4ccb968ab6dd7a351b6a14be5b7f8001e7c04540df7df6071`. Manifest tag dan digest menghasilkan HTTP `200` secara anonim dan digest identik.
...
### 5.3 Preflight dan siklus GPU pertama

- [x] Menyediakan `npm run runpod:preflight` yang hanya memakai REST API v2 `GET`, memeriksa konfigurasi lokal, dan menghasilkan JSON tanpa rahasia.
- [x] Menyediakan mode `--require-ready` untuk otomasi yang gagal ketika masih ada blocker.
- [x] Memeriksa inventaris, image digest, profil/cloud, harga terbaru, durasi, hard cost limit, data center/volume, dan akses direktori state.
- [x] Mewajibkan attestation saldo, scope key, satu replica, persistent state, watchdog selalu aktif, dan alert stop sebelum start, extend, atau admission job baru.
- [x] Memastikan stop darurat tetap tersedia ketika attestation kesiapan dicabut, selama write gateway dan API key valid.
- [x] Menyediakan runbook preflight dan template bukti siklus pertama untuk timeline, persistensi output, shutdown, serta rekonsiliasi invoice.
- [ ] Menyelesaikan seluruh blocker sampai preflight aktual menghasilkan `readyForPaidCycle: true`.
- [ ] Menjalankan dan mendokumentasikan satu siklus GPU nyata sesuai template bukti.

**Hasil Fase 5.3A tanpa saldo, 23 September 2026:**

- Preflight aktual berhasil membaca `0` Pod tanpa mutation. Snapshot Secure RTX 3090 menunjukkan `$0.50/jam`, availability `LOW`, dan estimasi compute 30 menit `$0.25` terhadap hard cost limit `$1.00`.
- `readyForPaidCycle` tetap `false` dengan sembilan blocker yang tepat: write lock, scope key, saldo, Network Volume, kecocokan data center, satu replica, persistent state, watchdog cloud, dan alert stop.
- Image immutable, inventaris kosong, API baca saja, harga, durasi, hard cost limit, serta akses direktori lokal sudah lulus.
- Controller memakai gate yang sama untuk biaya baru. Uji regresi membuktikan konfigurasi rencana yang hilang tidak menghalangi stop darurat Pod aktif.
- Tidak ada Pod, Network Volume, perubahan izin key, atau mutation RunPod yang dibuat.
...
- [x] Mendefinisikan ekspor/impor JSON berversi dari BEBAS dengan `expressionDialect: "bebas-v1"`, `script`, dan `caption`.
- [x] Menambahkan impor ke VoxCPM Studio tanpa membuat generator naskah lengkap kedua.
...
- [x] Mengekspor `Sample Audio 2` sebagai WAV PCM 16-bit mono 24 kHz tanpa normalisasi, meloloskan tujuh pemeriksaan lokal, dan menyimpannya sebagai referensi utama di Pustaka Suara browser.
...
| 24 September 2026 | Kesiapan lokal Fase 6D selesai tanpa saldo: pemeriksa tujuh aspek audio, panduan rekaman dan transkrip Hi-Fi, korpus 24 naskah, rubrik tujuh dimensi, serta template evaluasi CSV tersedia. Lima sampel pengguna dijadikan baseline tanpa menyalin audio ke repository. | 73/73 tes Node, TypeScript, ESLint, build Next.js, pemeriksaan UI desktop, serta [Application checks run 35987717585](https://github.com/Muhira007/VoxCPM-Studio/actions/runs/35987717585) lulus. Seluruh kasus korpus valid tanpa peringatan kompiler; laporan gagal tidak dapat disimpan dan laporan dengan status yang dimanipulasi ditolak. | Siapkan satu WAV PCM mono 15–60 detik dari bagian terbaik sampel. Saat saldo tersedia, jalankan lima kasus smoke dalam satu siklus GPU untuk menilai identitas, ekspresi, transisi, waktu, dan biaya aktual. |
| 24 September 2026 | Referensi utama lokal disiapkan dari `Sample Audio 2`: PCM 16-bit, mono, 24 kHz, 41,12 detik, RMS −19,8 dBFS, puncak −2,6 dBFS, clipping 0%, dan status **Siap diuji**. Referensi tersimpan di IndexedDB browser; berkas WAV berada di folder sampel pengguna dan tidak masuk Git. | 75/75 tes Node, TypeScript, ESLint, build Next.js, unggah melalui UI, persistensi setelah refresh, serta [Application checks run 35989386376](https://github.com/Muhira007/VoxCPM-Studio/actions/runs/35989386376) lulus. Cacat pembacaan sample rate akibat resampling Web Audio ditemukan dan diperbaiki agar metadata WAV asli 24 kHz yang dinilai. | Kesiapan tanpa saldo untuk referensi utama selesai. Langkah berikutnya memerlukan saldo RunPod: jalankan lima kasus smoke pada satu siklus GPU terkontrol, lalu nilai identitas dan ekspresi dengan rubrik. |

## 16. Langkah pengerjaan berikutnya

**Rekomendasi aktif setelah saldo tersedia:** gunakan referensi **Suara Saya — Referensi Utama** untuk kasus smoke `ID-01`, `ID-04`, `ID-14`, `ID-18`, dan `ID-22` pada satu siklus GPU terkontrol. Kelima kasus membatasi biaya awal sambil mencakup identitas, affiliate, rupiah, kata Inggris, dan transisi ekspresi. Bila kemiripan belum memadai, rekam ulang langsung sebagai WAV PCM mono agar sumber tidak berasal dari MP3.
